...
import {
  ...
  exportFavorites, selectLocalImage,
} from '@/api/backend';
          ...
          {filteredItems.map((item) => (
            <Card key={item.id} className="overflow-hidden">
              <div className="relative h-[160px] w-full overflow-hidden bg-surface-secondary">
                <img
                  src={item.preview_url || item.local_path || ''}
                  alt={item.title}
                  className="h-full w-full object-cover"
                  onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                />
                {item.local_path && <Badge className="absolute top-2 right-2">本地</Badge>}
              </div>
              ...
              <Card.Footer className="flex flex-wrap gap-1">
                ...
                <Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onPress={() => handleLocalize(item)}><Globe size={12} /> 本地化</Button>
