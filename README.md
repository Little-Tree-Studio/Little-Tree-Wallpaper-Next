...
> [!WARNING]
> 小树壁纸 Next 目前处于 Beta 测试阶段，功能、配置与扩展文件格式以及 API 均可能随时调整，并可能产生不兼容变更，不建议用于关键生产环境。动态壁纸、插件和高级自动化仍属于高级功能，使用前请阅读下方的[注意事项](#注意事项)。
...
- **本地桌面应用**：React 界面由 pywebview 承载，FastAPI 后端仅监听随机回环端口，并使用每次启动生成的令牌保护本地 API。
...
后端会在 `127.0.0.1` 上选择随机空闲端口，随后打开 pywebview 应用窗口。前端开发服务器不能独立替代桌面入口，因为大部分功能依赖本地后端 API。
