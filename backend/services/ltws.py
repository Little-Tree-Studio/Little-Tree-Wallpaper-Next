...
def _build_legacy_item_mapping(mapping_payload: dict[str, Any]) -> dict[str, str]:
    return {
...
class LTWSService:
    ...
    def create_source(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        source_dir = self.sources_dir / slugified_identifier
            ...
            try:
                ...
                if body_template not in {None, ""}:
                    ...
                    if body_type == "json":
                        ...
                        request_kwargs["data"] = rendered_body.encode("utf-8")
                response = self._executor.submit(requests.request, **request_kwargs).result()
                ...
                rule_indicates_retry = http_rule is not None and http_rule.get("retry_after") not in {None, ""}
                if attempt < max_attempts and (should_retry or rule_indicates_retry):
                    time.sleep(
